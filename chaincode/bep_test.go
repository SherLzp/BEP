...
func TestQueryBalanceByUserId(t *testing.T) {
	...
	res := mstub.MockInvoke("1", [][]byte{[]byte("QueryBalanceByUserId"), []byte("p1")})
	ast.Equal(int(res.Status), shim.OK, "Fail to query all the responses.")
	ast.NotNil(res.Payload, "query finds nothing!")
...
func TestQueryResponseByRequestId(t *testing.T) {
	...
	res := mstub.MockInvoke("1", [][]byte{[]byte("QueryResponseByUserId"), []byte("1")})
	ast.Equal(int(res.Status), shim.OK, "Fail to query all the responses.")
	ast.NotNil(res.Payload, "query finds nothing!")
	buffer := bytes.NewBuffer(res.Payload)
	Res := Response{}
	...
	for buffer.Len() > 0 {
		...
		case 1:
			...
			ast.Equal("2019-07-15-17-35", Res.CreateTime, "CreateTime not equal.")
		case 2:
			ast.Equal("1", Res.RequestId,  "RequestId not equal to 2.")
			ast.Equal("p3", Res.Owner, "Owner ID should be p2.")
			ast.Equal("p3_res1", Res.ResponseId, "Owner ID should be p1.")
			ast.Equal("I have a blue apple", Res.Answer, "Requirement not equal to apple.")
			ast.Equal("2019-07-15-17-35", Res.CreateTime, "CreateTime not equal.")
		}
		cnt += 1
	}
